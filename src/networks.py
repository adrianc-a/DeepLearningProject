...
class NetworkWrapper():
    def __init__(self, network, optimizer=None, new_sess=True):
        """
        ...
        with self.graph.as_default():
            ...
            if new_sess:
                ...
                self.train_step = optimizer.minimize(self.loss_function, var_list=vs, name='train_step')
                self.learning_phase = K.learning_phase()
                init_op = tf.global_variables_initializer()
                self.sess.run(init_op)
            else:
                self.train_step = network.train_step
    ...
    def restore(path):
        ...
        train_op = sess.graph.get_operation_by_name('train_step')

        loss = alphago_loss(pl_out, polY, v_out, valY)
        nn = namedtuple('Network',
                        'input policy_label value_label policy_output value_output loss learning_phase sess graph train_step')(
            *(inp, polY, valY, pl_out, v_out, loss, learning_phase, sess, g, train_op))

        return NetworkWrapper(nn, new_sess=False)
